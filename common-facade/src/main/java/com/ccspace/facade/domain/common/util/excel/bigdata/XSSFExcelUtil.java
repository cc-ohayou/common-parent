...
/**
 * @AUTHOR CF
 * @DATE Created on 2018/3/17 19:50.
 */
...
public class XSSFExcelUtil {
    ...
    public CellStyle makeHeadStyle() {
        CellStyle style = makeBodyStyle();
    ...
    public CellStyle makeBodyStyle() {
    ...
    /**
     * @description
     * @author CF create on 2018/3/21 14:40
     */
    public void setHeaderAndRow(List<Map<String, Object>> list,  SheetBean sb,Map<String,String> headers,
                                CellStyle headerStyle,CellStyle bodyStyle) throws Exception {
        Set<String> keys = headers.keySet();

        setHeaders(keys,headers,sb,headerStyle);
        List<RowBean> rowList = new ArrayList<>(1001);
        CellStyle style = bodyStyle==null?makeBodyStyle():bodyStyle;
        for (int i = 0; i <list.size() ; i++) {
            Iterator<String> headerNameIt  = keys.iterator();
            RowBean bodyRow = new RowBean();
            String           headerParam  ;    //存放临时键变量
            List<CellBean> bodyCellList = new  ArrayList<>(headers.size());

            while (headerNameIt.hasNext()) {
                headerParam = headerNameIt.next();
                CellBean bodyCell = new CellBean();
                bodyCell.setValue(list.get(i).get(headerParam));
                bodyCellList.add(bodyCell);
            }
            bodyRow.setCellList(bodyCellList);
            rowList.add(bodyRow);
            if(list.size()>1000){
            if(i!=0&&i%1000==0){
                try {
                    processWorkSheet(sb, rowList,style);
                } catch (Exception e) {
                    logger.warn("export settledList failed rowList error");
                }
                rowList.clear();
            }}
            if(i==list.size()-1){
        ...
        for (int j = 0; j <sheetMap.size() ; j++) {
            ((SXSSFSheet) sheetMap.get(j)).trackAllColumnsForAutoSizing();
            for(int i=0; i<headers.size();i++){
                sheetMap.get(j).autoSizeColumn(i);
            }
        }
    ...
    /**
     * @description 设置
     * @author CF create on 2018/3/21 14:39
     */
    private void setHeaders(Set<String> keys,Map<String, String> headers,  SheetBean sb,CellStyle headStyle) throws Exception {
        RowBean headerRow = new RowBean();
        List<RowBean> headerRowList = new ArrayList<>();
        List<CellBean> headerCellList = new ArrayList<>(40);
        CellStyle headerStyle = headStyle==null?makeHeadStyle():headStyle;
        Iterator<String> it1 = keys.iterator();
        String key ;    //存放临时键变量
        while (it1.hasNext()) {
            key = it1.next();
            CellBean cb = new CellBean();
            cb.setValue(headers.get(key));
            headerCellList.add(cb);
        }
        headerRow.setCellList(headerCellList);
        headerRowList.add(headerRow);
        processWorkSheet(sb, headerRowList,headerStyle);
        // 设定自动宽度
       /* for (int j = 0; j <sheetMap.size() ; j++) {
            ((SXSSFSheet) sheetMap.get(j)).trackAllColumnsForAutoSizing();
            for (int x = 0; x < headers.size(); x++) {
                sheetMap.get(j).autoSizeColumn(x);
            }
        }*/
    }
    public static void main(String[] args) throws Exception {
        XSSFExcelUtil g = new XSSFExcelUtil();
        ...
        FileOutputStream os = new FileOutputStream("G:\\fire-bird\\huge2.xlsx");
        g.write(os);
        os.flush();
        os.close();
        sw.stop();
