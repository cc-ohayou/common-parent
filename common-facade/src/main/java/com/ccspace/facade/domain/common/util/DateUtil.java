...
public class DateUtil {
    ...
    public static final String PARTTEN_NO_TIME = "yyyy-MM-dd";
    public static final String PARTTEN_NO_DATE = "HH:mm:ss";
    ...
    /**
     * @description 输出年月日时分
     * @author CF create on 2018/3/30 17:49
     */
     ...
     * @author CF create on 2018/6/26 11:36
     */
    ...
    public static String transferDateString(String dateStr){
        if(!StringUtils.isEmpty(dateStr)) {
            ...
            if (str[1].length() >= 2 && str[2].length() >= 2) {
                return dateStr;
            }
            str[1] = str[1].length() < 2 ? "0" + str[1] : str[1];
            str[2] = str[1].length() < 2 ? "0" + str[1] : str[2];
            dateStr = str[0] + "-" + str[1] + "-" + str[2];
        }
        return  dateStr;
    }
...
//					System.out.println("end:"+endNum);
    ...
    public static boolean compareDate(String timeStr) {
        ...
        try {
            ...
            log.error("compareDate-日期转化错误:", e.getStackTrace());
        }
        if (null == midDate) {
            return false;
        }
     ...
     * @return
     */
    public static Date strFormatDateDef(String str){
        try{
            SimpleDateFormat sdf = new SimpleDateFormat(PARTTEN_DEF);
            return sdf.parse(str);
        }catch (Exception e){
            throw new BusinessException("1000","时间格式转换异常");
        }
    }
    ...
    public static String dateHandle(Date date) {
        LocalDateTime oldTime = LocalDateTime.ofInstant(date.toInstant(), ZoneId.systemDefault());
        LocalDateTime nowTime = LocalDateTime.now();
        long minutes = ChronoUnit.MINUTES.between(oldTime, nowTime);
        long hours = ChronoUnit.HOURS.between(oldTime, nowTime);
        if (minutes == 0) {
            return CommentEnums.TIMEEMPTY1.getLabel();
        }
        if (minutes >= 1 && minutes < 60) {
            return minutes + CommentEnums.TIMEEMPTY2.getLabel();
        }
        if (hours >= 1 && hours <= 24) {
            return hours + CommentEnums.TIMEEMPTY3.getLabel();
        }
    ...
    /**
     * @description
     * @author CF create on 2018/8/29 17:09
     */
    ...
    public static Date parseStringToYmdFormat(String times){
        try {
            ...
            log.warn("###parseStringToYmdFormat  error",e);
        }
        return date;
    }
...
//        System.out.println(DateUtil.ymdFormat(DateUtil.getDaysBeforeAfterTime(-1)));
//        System.out.println(compareDate(" 15:20:30"));
//        System.out.println(LocalDateTime.parse("2018-08-31 00:00:00", newStandardFormat));
//        System.out.println(LocalDateTime.parse("2018-08-31 00:00:00", newStandardFormat).isAfter(LocalDateTime.now()));
//        Calendar c=Calendar.getInstance();
//        Date effectiveTime=new Date();
//        System.out.println(effectiveTime);
//        System.out.println( effectiveTime.getDay());
//        System.out.println(DateFormatUtils.format(1526541535000L, PARTTEN_DEF));

        Date now = new SimpleDateFormat(PARTTEN_DEF).parse("2019-01-10 10:00:00");
        DateUtil.isEffectiveTime(now, "09:00:00", "14:55:00");
     ...
     * 是否有效的时间（仅比较时间）
     * @param nowTime 当前时间
     * @param startTimeStr 开始时间 字符串 如 14:55:00
     * @param endTimeStr 结束时间 字符串 如 09:00:00
     * @return
     */
    public static boolean isEffectiveTime(Date nowTime, String startTimeStr, String endTimeStr) {
        Date effectiveStartTime = null;
        Date effectiveEndTime = null;
        try {
            effectiveStartTime = new SimpleDateFormat(PARTTEN_NO_DATE).parse(startTimeStr);
            effectiveEndTime = new SimpleDateFormat(PARTTEN_NO_DATE).parse(endTimeStr);
        } catch (ParseException e) {
            e.printStackTrace();
        }

        // 异常
        if (null == effectiveStartTime || effectiveEndTime == null) {
            return true;
        }
        ...
        start.setTime(effectiveStartTime);
    ...
    private enum CommentEnums {
        ...
        CommentEnums(Integer value, String label) {
            this.value = value;
            this.label = label;
        }
