...
import com.ccspace.facade.domain.common.util.RequestUtils;
...
/**
 * Create By GarryKing on 2017/9/12 下午1:49
 */
@Component
public class ControllerLogAspect extends MethodLogAspect {
    ...
    protected String extractLog(HttpServletRequest request, String error, Exception exception) {
        ...
        Object result = null;
        if (exception != null) {
            String message = StringUtils.isEmpty(error) ? "-" : error + "-" + exception.getMessage() + "-" +
    ...
    private String getLogMap(Object response, String methodName, String url, Object request, Object requestHeaders,
        ...
        // 执行API接口所耗费的时间
        map.put(CommonConstants.KEY_API_COST_TIME, getCostTime(startTime, endTime));
        // 请求的URL
        map.put(CommonConstants.KEY_API, url);
        // 请求的方法
        map.put(CommonConstants.KEY_METHOD, methodName);
        // 请求的参数
        map.put(CommonConstants.KEY_REQUEST, request);
        // 请求头
        map.put(CommonConstants.KEY_REQUEST_HEADER, requestHeaders);
        // 返回的结果
        map.put(CommonConstants.KEY_RESPONSE, response);
